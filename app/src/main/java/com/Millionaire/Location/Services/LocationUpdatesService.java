package com.Millionaire.Location.Services;
...
public class LocationUpdatesService extends Service {
    ...
    public void onCreate() {
        ...
        userList = AppDatabase.getInstance(getBaseContext()).getuserDA().getAll();
        if (userList.size() != 0 ) {
            user = userList.get(0);
        }
    ...
    private Notification getNotification() {
        ...
        if (mLocation == null || ( mLocation.getLatitude() == 0 && mLocation.getLongitude() == 0) ) {
            Toast.makeText(this, "لوکیشن وجود ندارد", Toast.LENGTH_SHORT).show();
        } else {
            AppDatabase.getInstance(LocationUpdatesService.this)
                    .getlocationUser().SetInsert(new LocationUser(mLocation.getLatitude()
                    , mLocation.getLongitude() , Setting.getDateToday(), String.valueOf(Setting.getTime()) , false , false));
            if (Utils.setCheckNetWork(getBaseContext())) {
                locationUsers = AppDatabase.getInstance(getBaseContext()).getlocationUser().getListSendServer(true);
        ...
        CharSequence text = Utils.getLocationText(mLocation ,LocationUpdatesService.this);
        ...
        NotificationCompat.Builder builder = new NotificationCompat.Builder(this)
                .addAction(R.drawable.ic_search, getString(R.string.launch_activity),
    ...
    public void sendLocationToServer(final int i , final String userCode , final List<LocationUser> locationUsers) {
        ApiInterface apiInterface = ApiClient.getRetrofit(getBaseContext()).create(ApiInterface.class);
                ...
                public void onResponse(Call<String> call, Response<String> response) {
                    if (response.isSuccessful()) {
                        if (response.body() == null) {
                            ...
                            AppDatabase.getInstance(getBaseContext()).getlocationUser().update(true, locationUsers.get(i).getId());
